{if $mosDesign} 
    {include file="$base_template_path/std_mos/std_service.tpl"}
{else}
...
<fieldset class="form-block {if $block_class}{$block_class}{/if}">
	{if isset($label)&&$label}<legend>{$label}</legend>{/if}

<div class="wrap row service_block {if isset($container_class)} {$container_class}{/if} {if isset($required)&&$required}required{/if}"  id="{if isset($container_id)&&$container_id}{$container_id}{else}service{/if}">
	...
	<legend class="col-md-11 col-sm-11 col-xs-10">{if isset($label_service)&&$label_service}{$label_service}{else}Популярные услуги:{/if}</legend>
...
<fieldset class="form-block hidden no-legend">
...
var {/literal}{if isset($container_id)&&$container_id}{$container_id}{else}service{/if} = {$items|json_encode};{literal}
		...
		$('.service_block .holder a').off('click.service').on('click.service', function(){
			...
			{/literal}{if !empty($service_callback)}
				...
				if (typeof $service_callback ==='function') $service_callback($(this));
			{/if}{literal}
