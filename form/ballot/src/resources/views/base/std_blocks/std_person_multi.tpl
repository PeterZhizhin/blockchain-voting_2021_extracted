{if $mosDesign}
    {include file="$base_template_path/std_mos/std_person_multi.tpl"}
{else}
<script type="text/html" id="pmulti_{$block_id}">
	...
	{/if}
	{if isset($subtitle)&&$subtitle}<legend class="col-md-10">{$subtitle}</legend>{/if}
	{if $send_counter}
	...
	{/if}
            <div class="col-md-offset-10 col-md-2 col-sm-12 col-xs-12 pmulti-del {if !$del_text}hidden{/if}"  data-template-id="pmulti_{$block_id}" serial="{if isset($count_to_add)&&$count_to_add}{$count_to_add}{else}1{/if}" count="<%=multiblockCounter%>" bid="{$block_id}">
