<div class="form-horizoontal several-inline row wrap{if $container_class} {$container_class}{/if}"{if $container_id} id="{$container_id}"{/if} >
    {if isset($label)&&$label}<label class="col-md-3 col-sm-4 col-xs-10">{$label}{if isset($text_required) && $text_required}<span class="required">*</span>{/if}</label>{/if}
    ...
    <div style="padding-left: 7px; padding-right: 8px;" class="holder {if !$select_label && !$select_no_row}col-md-offset-3 col-sm-offset-4{/if} {if $select_no_row}col-md-3 col-sm-3 col-xs-10{else}col-md-2 col-sm-7 {if !$select_label}col-xs-10{else}col-xs-12{/if} {/if}">
