{if $mosDesign}
    {include file="$base_template_path/std_mos/std_declarant_and_trustee.tpl"}
{else}
...
{* Новый справочник организационно-правовых форм *}
{if (isset($newOrgFormReference))} {assign var=orgFormReferenceName value='OKOPF'}{else}{assign var=orgFormReferenceName value='DTIU_030301_OKOPF_CODES'}{/if}
...
{if $person_linked.$person_type|count>0}
	<fieldset class="form-block cat_enjoy">
		...
		<div class="hidden visual visual_2">
			{* ФИЗ *}
			{if $person_type==1}
                <legend>{if $trustee_legend_1}{$trustee_legend_1}{else}Сведения об уполномоченном физическом лице{/if}</legend>
				{include file="$person_block_tmpl" container_class="" show_email=$person_show_email show_phone=$person_show_phone show_work_phone="{if isset($person_show_work_phone)}{$person_show_work_phone}{else}0{/if}" phone_work_label="{if isset($phone_work_label)&&$phone_work_label}{$phone_work_label}{else}Телефон{/if}" person="trustee"  show_gender="{if isset($person_show_gender)}{$person_show_gender}{else}0{/if}" show_birthdate=$person_show_birthdate show_snils=$person_show_snils autocomplete=true maxlength_lastname=$maxlength_lastname maxlength_firstname=$maxlength_firstname maxlength_middlename=$maxlength_middlename }
				{if !isset($hide_person_document_trustee) || !$hide_person_document_trustee}{include file="$base_template_path/std_blocks/std_person_document.tpl" portalid="trustee" wrapper="div" document_types=$document_types  autocomplete=true container_class="" contact="trustee" title="Документ, удостоверяющий личность" required=true no_birthday="1" free_photo=false one_photo="0" no_photo="1" no_ovdcode="{if isset($declarant_show_ovdcode)}{!$declarant_show_ovdcode}{else}1{/if}" showSavedDocs="1" savedDocsReadonly="0" docTypeList="{if $declarant_docTypeList}{$declarant_docTypeList}{else}full{/if}"}{/if}
			{/if}
			{*	ЮЛ	*}
			{if $person_type==2}
				<legend>{if $trustee_legend_2}{$trustee_legend_2}{else}Сведения об уполномоченной организации (юридическом лице){/if}</legend>
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_name_box" label="Полное наименование" maxlength="{$maxlength_full_name}" required=true name="field[{if isset($custom)&&$custom}new_fullname{else}trustee.new_full_name{/if}]" validator="" autocomplete_from="LEGAL_REG_INFO:FULL_NAME"}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_name_box" label="Сокращенное наименование" maxlength="250" required=true name="field[{if isset($custom)&&$custom}new_name{else}trustee.name{/if}]" validator=""  autocomplete_from="LEGAL_REG_INFO:SHORT_NAME"}
				{if (isset($show_BrandName) && $show_BrandName)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_BrandName_box" maxlength="250" label="Фирменное наименование" required=false name="field[{if isset($custom)&&$custom}new_brandname{else}trustee.new_brandname{/if}]" autocomplete_from="LEGAL_REG_INFO:SHOW_NAME"}{/if}{*autocomplete_from="LEGAL_INFO:SHOW_NAME"*}
				{if (isset($show_OrgFormCode) && $show_OrgFormCode)}
					{include file="$base_template_path/std_blocks/std_select.tpl" context_search=true container_class="trustee_org_OrgFormCode_box hidden elk_loaded cat cat2" label="Организационно-правовая форма" required=true name="field[{if isset($custom)&&$custom}new_okopf{else}trustee.new_okopf{/if}]" autocomplete_from="{if (!isset($newOrgFormReference))}LEGAL_REG_INFO:OPF{/if}" }
					...
					{*</script>*}
				{/if}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_inn_box" label="ИНН" required=true name="field[{if isset($custom)&&$custom}new_inn{else}trustee.new_inn{/if}]" validator="inn_ul" maxlength="10" mask="9999999999" autocomplete_from="LEGAL_REG_INFO:INN"}
				{*Инфоблок перед полями ЮЛ*}
				{if isset($show_trustee_ul_infoblock) && $show_trustee_ul_infoblock}{include file="$base_template_path/std_blocks/std_infoblock.tpl" container_class="hidden cat cat2 trustee_ul_infoblock" color="orange"  text="{$trustee_ul_infoblock_text}"}{/if}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_ogrn_box" label="ОГРН" required=true name="field[{if isset($custom)&&$custom}new_ogrn{else}trustee.new_ogrn{/if}]" maxlength="13" mask="9999999999999" validator="check_ogrn" autocomplete_from="LEGAL_REG_INFO:OGRN"}

				{if (isset($show_kpp) && $show_kpp)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_kpp_box decl hidden" label="КПП" required=true name="field[{if isset($custom)&&$custom}new_kpp{else}trustee.new_kpp{/if}]" minlength="9" validator="kpp" maxlength="9" mask="9999bb999" hint="997950001" autocomplete_from="LEGAL_REG_INFO:KPP"}{/if}  {*autocomplete_from="LEGAL_REG_INFO:KPP"*}
				{if (isset($show_okpo) && $show_okpo)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_okpo_box decl hidden" label="ОКПО" required=true name="field[{if isset($custom)&&$custom}new_okpo{else}trustee.new_okpo{/if}]" minlength="8" validator="okpo" maxlength="10" mask="9|repeat" hint="09808117"}{/if}
				{if (isset($show_okonh) && $show_okonh)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_org_okonx_box decl hidden" label="ОКОНХ" required=true name="field[{if isset($custom)&&$custom}new_okonh{else}trustee.new_okonh{/if}]"  minlength="5" validator="" maxlength="5" mask="9|repeat" hint="84300"}{/if}

			{/if}
			{*	ИП	*}
			{if $person_type==3}
				<legend>{if $trustee_legend_3}{$trustee_legend_3}{else}Сведения об уполномоченной организации (индивидуальном предпринимателе){/if}</legend>
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_ip_lastname_box" label="Фамилия" required=true name="field[{if isset($custom)&&$custom}new_lastname{else}trustee_head.lastname{/if}]" autocomplete_from="FIO:SURNAME" maxlength=$maxlength_lastname validator="fio"}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_ip_firstname_box" label="Имя" required=true name="field[{if isset($custom)&&$custom}new_firstname{else}trustee_head.firstname{/if}]" autocomplete_from="FIO:NAME" maxlength=$maxlength_firstname validator="fio"}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_ip_middlename_box" label="Отчество" required=false name="field[{if isset($custom)&&$custom}new_middlename{else}trustee_head.middlename{/if}]" autocomplete_from="FIO:PATRONYMIC" maxlength=$maxlength_middlename validator="fio"}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_ip_inn_box" label="ИНН" required=true name="field[{if isset($custom)&&$custom}new_inn{else}trustee.new_inn{/if}]" autocomplete_from="LEGAL_REG_INFO:INN" validator="inn_fiz" maxlength="12" mask="999999999999"}
				{include file="$base_template_path/std_blocks/std_text.tpl" container_class="trustee_ip_ogrnip_box" label="ОГРНИП" required=true name="field[{if isset($custom)&&$custom}new_ogrn{else}trustee.new_ogrn{/if}]" autocomplete_from="LEGAL_REG_INFO:OGRN" maxlength="15" mask="999999999999999" validator="check_ogrnip"}
			{/if}
			{if $person_type>1}
			...
			{if $person_type==2}
				<div class="">
					...
					{/if}
					{if !isset($hide_info) || !$hide_info}{include file="$base_template_path/std_blocks/std_infoblock.tpl" container_class="visual visual_2 hidden" color="orange"  text="{if isset($file_document_trustee_ul.infoblock)}{$file_document_trustee_ul.infoblock}{else}К заявлению необходимо прикрепить все доверенности и иные документы, подписанные электронной подписью доверителя или нотариуса, подтверждающие Ваши полномочия на подачу заявления от имени организации.{/if}"}{/if}
					{if !isset($hide_doc) || !$hide_doc}
...
{/if}
<fieldset class="form-block cat cat1 cat2 cat3 pers_block">
	<legend>{if $declarant_legend}{$declarant_legend}{else}Сведения о заявителе{/if}</legend>
	...
	{if !isset($person_linked[$person_type])||'2'|in_array:$person_linked[$person_type]||$person_linked.$person_type|count==0}
		{*	ЮЛ	*}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_name_box elk_loaded hidden cat cat2" label="Полное наименование" maxlength="{$maxlength_full_name}" required=true validator="" name="field[account.new_full_name]" autocomplete_from="LEGAL_REG_INFO:FULL_NAME"}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_name_box elk_loaded hidden cat cat2" label="Сокращенное наименование" maxlength="250" required=true validator="" name="field[account.name]" autocomplete_from="LEGAL_REG_INFO:SHORT_NAME"}
		{if (isset($show_BrandName) && $show_BrandName)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_BrandName_box hidden cat cat2" maxlength="250" label="Фирменное наименование" required=false name="field[account.new_brandname]" autocomplete_from="LEGAL_REG_INFO:SHOW_NAME"}{/if}{* autocomplete_from="LEGAL_INFO:SHOW_NAME*}
		
		{if (isset($show_OrgFormCode) && $show_OrgFormCode)}
			{include file="$base_template_path/std_blocks/std_select.tpl" context_search=true value=$show_OrgFormCode container_class="org_OrgFormCode_box hidden elk_loaded cat cat2" label="Организационно-правовая форма" required=true name="field[account.new_okopf]" autocomplete_from="{if (!isset($newOrgFormReference))}LEGAL_REG_INFO:OPF{/if}"}
			{*<script type="text/javascript">*}
				{*$(function() {*}
		...
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_inn_box elk_loaded hidden cat cat2" label="ИНН" required=true name="field[account.new_inn]" validator="inn_ul" maxlength="10" mask="9999999999" autocomplete_from="LEGAL_REG_INFO:INN"}
		{*Инфоблок перед полями ЮЛ*}
		{if isset($show_declarant_ul_infoblock) && $show_declarant_ul_infoblock}{include file="$base_template_path/std_blocks/std_infoblock.tpl" container_class="hidden cat cat2 declarant_ul_infoblock" color="orange"  text="{$declarant_ul_infoblock_text}"}{/if}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_ogrn_box elk_loaded hidden cat cat2" label="ОГРН" required=true name="field[account.new_ogrn]" maxlength="13" mask="9999999999999" validator="check_ogrn" autocomplete_from="LEGAL_REG_INFO:OGRN"}
		{if (isset($show_kpp) && $show_kpp)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_kpp_box hidden elk_loaded cat cat2" label="КПП" required=true name="field[account.new_kpp]" minlength="9" validator="kpp" maxlength="9" mask="9999bb999" hint="997950001" autocomplete_from="LEGAL_REG_INFO:KPP"}{/if} {*autocomplete_from="LEGAL_REG_INFO:KPP"*}
		{if (isset($show_okpo) && $show_okpo)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_okpo_box decl hidden cat cat2" label="ОКПО" required=true name="field[account.new_okpo]" minlength="8" validator="okpo" maxlength="10" mask="9|repeat" hint="09808117"}{/if}
		{if (isset($show_okved) && $show_okved)}
			{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_okved_box elk_loaded hidden cat cat2" label="ОКВЭД" maxlength="8" minlength="5" mask="99.99.99" required=true validator="" name="field[account.new_okved]" autocomplete_from=""}
		{/if}
		{if (isset($show_okonh) && $show_okonh)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_okonx_box decl hidden cat cat2" label="ОКОНХ" required=true name="field[new_decl_okonh]"  minlength="5" validator="" maxlength="5" mask="9|repeat" hint="84300"}{/if}
		{if (isset($show_website) && $show_website)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_website_box hidden elk_loaded cat cat2" maxlength="250" label="Адрес официального сайта" required={$required_website} name="field[account.websiteurl]" hint={$hint_website_text}  }{/if}
	...
	{if !isset($person_linked[$person_type])||'3'|in_array:$person_linked[$person_type]||$person_linked.$person_type|count==0} 
		{*	ИП	*}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="ip_lastname_box elk_loaded hidden cat cat3" label="Фамилия" required=true name="field[ip.lastname]" autocomplete_from="FIO:SURNAME" maxlength=$maxlength_lastname validator="fio"}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="ip_firstname_box elk_loaded hidden cat cat3" label="Имя" required=true name="field[ip.firstname]" autocomplete_from="FIO:NAME" maxlength=$maxlength_firstname validator="fio"}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="ip_middlename_box elk_loaded hidden cat cat3" label="Отчество" required=false name="field[ip.middlename]" autocomplete_from="FIO:PATRONYMIC" maxlength=$maxlength_middlename validator="fio"}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="ip_inn_box hidden elk_loaded cat cat3" label="ИНН" required=true name="field[ip.new_inn]" autocomplete_from="LEGAL_REG_INFO:INN" validator="inn_fiz" maxlength="12" mask="999999999999"}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="ip_ogrnip_box hidden elk_loaded cat cat3" label="ОГРНИП" required=true name="field[ip.new_ogrn]" autocomplete_from="LEGAL_REG_INFO:OGRN" maxlength="15" mask="999999999999999" validator="check_ogrnip"}
		{if (isset($show_website) && $show_website)}{include file="$base_template_path/std_blocks/std_text.tpl" container_class="org_website_box hidden elk_loaded cat cat3" maxlength="250" label="Адрес официального сайта" required=false name="field[account.websiteurl_double]"  }{/if}
		{if (isset($newOrgFormReference))}
			<input type="hidden" name="field[internal.is_new_reference]" value="1">
		{/if}
	{/if}
	...
	{if $person_show_phone}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="cat contact_phone cat2 cat3 elk_loaded" label="Контактный телефон" required=true name="field[account.telephone1]" autocomplete_from="LEGAL_INFO:PHONE" validator="phone_num" mask="(999) 999-99-99" placeholder="(926) 123-45-67" }
	{/if}
	{if $person_show_email}
		{include file="$base_template_path/std_blocks/std_text.tpl" container_class="cat contact_email cat2 cat3 elk_loaded" label="Адрес электронной почты" required=true name="field[account.emailaddress1]" autocomplete_from="LEGAL_INFO:EMAIL" validator="email" placeholder="login@mail.ru" maxlength="100"}
	{/if}
	...
	<div class="cat only_person cat2">
		{if !isset($hide_head_type) || !$hide_head_type}
			...
			{include file="$base_template_path/std_blocks/std_radiogroup.tpl" container_class="visual_controller" required=true items=$type_yavl label="{if $field_label_ul_declarant}{$field_label_ul_declarant}{else}Вы являетесь:{/if}" name=$field_name_ul_declarant  label_position="top"}
		{/if}
		{if !isset($hide_info) || !$hide_info}{include file="$base_template_path/std_blocks/std_infoblock.tpl" container_class="visual visual_2 hidden" color="orange"  text="{if isset($file_document_declarant_ul.infoblock)}{$file_document_declarant_ul.infoblock}{else}К заявлению необходимо прикрепить все доверенности и иные документы, подписанные электронной подписью доверителя или нотариуса, подтверждающие Ваши полномочия на подачу заявления от имени организации.{/if}"}{/if}
		{if !isset($hide_doc) || !$hide_doc}
...
<script type="text/javascript">
	...
	$(function() {
                ...
                Lib.getReference({$orgFormReferenceName|json_encode},'.trustee_org_OrgFormCode_box'); //справочники проинициализируем
                Lib.getReference({$orgFormReferenceName|json_encode},'.org_OrgFormCode_box'); //справочники проинициализируем
		function change_cat() {
			...
			$('#cat').val(type);
			$('.pers_block .cat').hide();
{*			if ($('.type_decl input:radio:checked').length > 0)
				var type = $('.type_decl input:radio:checked').val();
			else
				var type = 0;
			if ($('.type_decl_person input:radio:checked').length > 0)
				var type_decl_person = $('.type_decl_person input:radio:checked').val();
			else
				var type_decl_person = 0;*}


			$('.pers_block .cat' + $('#cat').val()).show();
{*			if (type == 2)
				$('.type_decl_person').closest('.form-block').show();

			if (type == 1 && person_type == 2)
				$('.type_yavl').show();*}
			//console.log('Тип заявителя:'+cat)
