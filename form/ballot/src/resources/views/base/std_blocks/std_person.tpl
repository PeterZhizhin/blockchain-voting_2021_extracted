{if $mosDesign}
    {include file="$base_template_path/std_mos/std_person.tpl" show_middlename_checkbox=false}
{else}
	...
	{$age=false}
	{include file="$base_template_path/std_blocks/std_text.tpl" validator="fio" label="Фамилия" required=true class="{$surname_class}" name="field[{$person_new}{$prefix}lastname{$postfix}]" hint="{if isset($lastname_hint)&&$lastname_hint}{$lastname_hint}{else}false{/if}" age=$person_age container_class="lastname {$container_class}" id="{$person}-lastname{$postfix}" maxlength="{if $maxlength_lastname}{$maxlength_lastname}{else}50{/if}" disabled={(isset($disabled)&&$disabled)?'disabled':false} autocomplete_from="{if $autocomplete}{if $child}CHILDREN:SURNAME{else}{if $complex}COMPLEX_OMS:SURNAME{else}FIO:SURNAME{/if}{/if}{/if}" }
	{include file="$base_template_path/std_blocks/std_text.tpl" validator="fio" label="Имя" required=true class="{$firstname_class}" name="field[{$person_new}{$prefix}firstname{$postfix}]" hint="{if isset($firstname_hint)&&$firstname_hint}{$firstname_hint}{else}false{/if}" container_class="firstname {$container_class}" id="{$person}-firstname{$postfix}" maxlength="{if $maxlength_firstname}{$maxlength_firstname}{else}50{/if}" disabled={(isset($disabled)&&$disabled)?'disabled':false} autocomplete_from="{if $autocomplete}{if $child}CHILDREN:NAME{else}{if $complex}COMPLEX_OMS:NAME{else}FIO:NAME{/if}{/if}{/if}"}
	...
	{if !isset($show_gender) || $show_gender}
		{include file="$base_template_path/std_blocks/std_radiogroup.tpl" layout='horizontal' items=$gender_dict label="Пол" required=true name="field[{$person_new}gendercode{$postfix}]" hint="{if isset($gender_hint)&&$gender_hint}{$gender_hint}{else}false{/if}" container_class="gender {$person}-gender {$container_class}" id="{$person}-gender{$postfix}" autocomplete_from="{if $autocomplete}{if $child}CHILDREN:GENDER{else}{if $complex}COMPLEX_OMS:GENDER{else}PERSON:GENDER{/if}{/if}{/if}" disabled={(isset($disabled)&&$disabled)?'disabled':false}}
	{/if}
	{if !isset($show_birthdate) || $show_birthdate}
		{include file="$base_template_path/std_blocks/std_date.tpl" label="Дата рождения" class="{$birthdate_class}" required=true name="{if $birthdate_name}{$birthdate_name}{else}field[{$person_new}birthdate{$postfix}]{/if}" container_class="birthdate {$person}-birthdate {$container_class}" id="{if $birthdate_id}{$birthdate_id}{else}{$person}-birthdate{/if}{$postfix}" validator="date_in_past_and_now {$birthdate_validator}" autocomplete_from="{($autocomplete) ? (($child) ? 'CHILDREN:BIRTHDATE' : (($complex) ? 'COMPLEX_OMS:BIRTHDATE' : 'PERSON:BIRTHDATE')) : false}" disabled={(isset($disabled)&&$disabled)?'disabled':false} hint="{if isset($birthdate_hint)&&$birthdate_hint}{$birthdate_hint}{else}false{/if}"}
	{/if}
	{if isset($show_birthplace) || $show_birthplace}
		{include file="$base_template_path/std_blocks/std_text.tpl" class="{$birthplace_class}" label="Место рождения" container_class="birthplace {$person}-birthplace {$container_class}" hint="{if isset($birthplace_hint)&&$birthplace_hint}{$birthplace_hint}{else}г. Ангарск{/if}" required=true name="field[{$person_new}new_birthplace{$postfix}]" disabled={(isset($disabled)&&$disabled)?'disabled':false} autocomplete_from="{if $autocomplete}{if $child}{else}PASSPORT_RF:BIRTHPLACE{/if}{/if}" maxlength="{if $maxlength_birthplace}{$maxlength_birthplace}{else}200{/if}"}
		{/if}
	...
	{if !isset($show_snils) || $show_snils}
		{include file="$base_template_path/std_blocks/std_snils.tpl" label="СНИЛС" required=$snils_required name="field[{$person_new}new_snils{$postfix}]" container_class="snils {$person}-snils {$container_class}" id="{$person}-snils{$postfix}" autocomplete_from="{if $autocomplete}{if $child}CHILDREN:SNILS_NUMBER{else}SNILS:SNILS{/if}{/if}" disabled={(isset($disabled)&&$disabled)?'disabled':false}}
	{/if}
{/if}