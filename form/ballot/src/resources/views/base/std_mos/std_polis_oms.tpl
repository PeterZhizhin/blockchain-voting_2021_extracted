...
{if $complex}
    ...
    {$autoCompleteBirthday = "{$block_from}:BIRTHDATE"}
    {$many = true}
...
{elseif $isChild}
    ...
    {$origin = 'CHILD'}
{else}
    {$autoCompleteBirthday = "PERSON:BIRTHDATE"}
...
{if $contact||$birthdate_name}<div data-elk-block="{$block_from}" >{/if}
    ...
    {if $contact}
        ...
        {else}
            {include
                ...
                autocomplete_from=$autoCompleteBirthday
        ...
        {include
            file="$base_template_path/std_mos/std_person.tpl"
            person_age=$age
            ...
            person=$contact
            show_birthdate="{if $masterField=='FIO'&&$many}1{else}0{/if}"
            no_ovdcode=false
            autocomplete=true
            mask=false
            minlength=false
    ...
    {elseif $birthdate_name}
        ...
        {include
            file="$base_template_path/std_mos/std_date.tpl"
            label="Дата рождения"
