...
{if !$no_row}{$no_row=false}{/if}
{$age=false}
{if $no_row!=true}<div class="row first-row">{/if}
	...
	<div class="col-md-4 form-group middlename_block">
		{block name="middlename_block"}                    
			...
			autocomplete_from="{if $autocomplete}{if $child}CHILDREN:PATRONYMIC{else}{if $complex}COMPLEX_OMS:PATRONYMIC{else}FIO:PATRONYMIC{/if}{/if}{/if}"
                        link="{if !isset($show_middlename_checkbox) || $show_middlename_checkbox}<a href=\"#\" class=\"no_middlename\">Нет отчества</a>{/if}"                                 
                        }
		{/block}               
	</div>
...
{if !isset($show_gender) || $show_gender}
	<div class="form-group col-md-4 padding_l_r_0">
		{include file="$base_template_path/std_blocks/std_radiogroup.tpl" layout='horizontal' items=$gender_dict label="Пол:" required=true name="field[{$person_new}gendercode{$postfix}]" hint="{if isset($gender_hint)&&$gender_hint}{$gender_hint}{else}false{/if}" container_class="gender {$person}-gender {$container_class}" container_class_for_error="col-md-12" id="{$person}-gender{$postfix}" autocomplete_from="{if $autocomplete}{if $child}CHILDREN:GENDER{else}{if $complex}COMPLEX_OMS:GENDER{else}PERSON:GENDER{/if}{/if}{/if}" disabled={(isset($disabled)&&$disabled)?'disabled':false} label_position="top"}
	</div>
{/if}
