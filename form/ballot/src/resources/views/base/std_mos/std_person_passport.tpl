{if !isset($document_date_validator)||(isset($document_date_validator)&&!$document_date_validator)}
	...
	{assign var="container_class" value=""}
        <div class="row">
	{include file="$base_template_path/std_blocks/std_serial_number.tpl" label="Серия" label2="Номер" serial_required=true hint="4510" hint2="123456" serial_class='document_serial' number_class='document_number' serial_minlength=4 serial_maxlength=4 serial_mask="9999" number_minlength=6 number_maxlength=6 number_mask="999999" required=true id="{$contact}Series" number_id="{$contact}Number{$postfix}" container_id="row{$contact}new_passport_serie{$postfix}" serial_name="field[{$contact}.new_{if $v6}serie{else}passport_serie{/if}{$postfix}]" number_required=true number_container_id="row{$contact}new_passport{$postfix}" number_name="field[{$contact}.new_{if $v6}name{else}passport{/if}{$postfix}]" autocomplete_from={($autocomplete)?(($child)?"CHILDREN:BIRTH_CERT_NUMBER":"PASSPORT_RF:NUMBER"):""}}
	{if !isset($no_date)||!$no_date}
		{include file="$base_template_path/std_blocks/std_date.tpl" label="Когда выдан" container_class="col-md-4 form-group" class="document_date" required=true hint="17.06.2000" id="{$contact}Date{$postfix}" container_id="row{$contact}new_passport_date{$postfix}" name="field[{$contact}.new_{if $v6}docdate{else}passport_date{/if}{$postfix}]" validator="date_in_past_and_now {$document_date_validator}" autocomplete_from={($autocomplete)?(($child)?'CHILDREN:BIRTH_CERT_DATE':"PASSPORT_RF:ISSUED_ON"):""}}
	{/if}
        </div>
        <div class="row">
	{if !isset($no_place)||!$no_place}
                
		{include file="$base_template_path/std_blocks/std_text.tpl" label="Кем выдан" container_class="col-md-8 form-group field--document--place" class="document_place" validator="{$no_place_validator}" required=true hint="ОУФМС города Москвы по району Академический" container_id="row{$contact}new_passport_place{$postfix}" name="field[{$contact}.new_{if $v6}whosign{else}passport_place{/if}{$postfix}]" maxlength="250" autocomplete_from={($autocomplete)?(($child)?'CHILDREN:BERTH_CERT_ISSUED_BY':"PASSPORT_RF:ISSUED_BY"):""}}
	{/if}

	{if !$no_ovdcode}
		{include file="$base_template_path/std_blocks/std_text.tpl" class="document_ovdcode" container_class="col-md-4 form-group" label="Код подразделения" mask="999-999" hint="111-222" required=true id="{$contact}DivisionCode{$postfix}" container_id="row{$contact}new_divisioncode{$postfix}" name="field[{$contact}.new_divisioncode{$postfix}]" autocomplete_from={($autocomplete)?(($child)?'CHILDREN:DIVISION_CODE':"PASSPORT_RF:DIVISION_CODE"):""}}
               
        {/if}
        </div>
        <div class="row">
        {if $validityperiod}
		{include file="$base_template_path/std_blocks/std_date.tpl" label="Срок действия" container_class="col-md-4 form-group" class="document_validityperiod" required=true id="{$contact}Validityperiod{$postfix}" container_id="row{$contact}new_validityperiod{$postfix}" name="field[{$contact}.new_validityperiod{$postfix}]" validator="date_in_future"}
	{/if}
	{if !$no_birthday}
		{include file="$base_template_path/std_blocks/std_date.tpl" class="document_birthdate" container_class="col-md-4 form-group" label="Дата рождения" hint="11.01.1963" required=true id="{$contact}.birth_date{$postfix}" name="field[{$contact}.birthdate{$postfix}]" validator="date_in_past_and_now" autocomplete_from={($autocomplete)?"PERSON:BIRTHDATE":""}}

		{if !$no_birthplace}
			{include file="$base_template_path/std_blocks/std_text.tpl" container_class="col-md-8 form-group" class="document_birthdateplace" validator="main_ru" label="Место рождения" hint="{if isset($birthplace_hint)&&$birthplace_hint}{$birthplace_hint}{else}г. Ангарск{/if}" required=true id="{$contact}BirthPlace{$postfix}" name="{if isset($birthplace_name)&&$birthplace_name}field[{$contact}.{$birthplace_name}]{else}field[{$contact}.new_birthplace{$postfix}]{/if}" autocomplete_from={($autocomplete)?"PASSPORT_RF:BIRTHPLACE":""} maxlength=200}
		{/if}
	{else}
		{if isset($no_birthplace) && !$no_birthplace}
			{include file="$base_template_path/std_blocks/std_text.tpl" container_class="col-md-8 form-group" class="document_birthdateplace" validator="main_ru"  label="Место рождения" hint="{if isset($birthplace_hint)&&$birthplace_hint}{$birthplace_hint}{else}г. Ангарск{/if}" required=true id="{$contact}BirthPlace{$postfix}" name="{if isset($birthplace_name)&&$birthplace_name}field[{$contact}.{$birthplace_name}]{else}field[{$contact}.new_birthplace{$postfix}]{/if}" autocomplete_from={($autocomplete)?(($child)?'':"PASSPORT_RF:BIRTHPLACE"):""} maxlength=200}
		{/if}
	{/if}
	{if $photo_info_text}
	...
	{if !$no_photo}
		...
		{if !$one_photo}
			{include file="$base_template_path/std_blocks/std_file.tpl" class="document_photopage" label="Страница паспорта с фотографией" required=!$free_photo name="field[{$contact}.0.contact_annotation.1.documentbody{$postfix}]" hint="Прикрепите электронный образ документа" container_class=" margin_l_r_0"}
			{include file="$base_template_path/std_blocks/std_file.tpl" class="document_regpage" label="{if $regpage_label}{$regpage_label}{else}Страница паспорта с адресом регистрации{/if}" required=!$free_photo name="field[{$contact}.0.contact_annotation.2.documentbody{$postfix}]" hint="Прикрепите электронный образ документа"  container_class="margin_l_r_0"}
		{else}
	...
	{if $document}
		...
		{else}
		{if isset($type_decl)} <div class='row'> {/if}
			{include file="$base_template_path/std_blocks/std_document.tpl"
						label="{if $document_label}{$document_label}{else}Электронный образ{/if}"
						class=$document_class
						container_id="{$container_id}_ebody{$postfix}"
