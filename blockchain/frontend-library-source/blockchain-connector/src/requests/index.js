...
const ReadPostRequest = require('../readPostRequest');
 ...
 * @property {BallotStatus} status - ballot status
 */

/**
 * @typedef DistrictResult
 * @type {Object}
 * @property {number} district_id - district id
 ...
 * @type {Object}
 * @property {Object.<number, DistrictResult} district_results - results map (<districtId, DistrictResult>)
 * @property {number} invalid_ballots_amount - amount of voting's invalid ballots
 * @property {number} unique_valid_ballots_amount - amount of voting's unique valid ballots
 */
...
module.exports = {
  ...
  /**
   * Makes ReadRequest returning stored ballots amount for whole voting
   * @param {string} votingId - voting Id
   * @return {ReadRequest<number>} - ReadRequest returning stored ballots amount
   */
   ...
   * by district map
   */
  getStoredBallotsAmountByDistrict: (votingId) => {
    return new ReadRequest(
      [
        'services/votings_service/v1/stored-ballots-amount-by-district',
        `?voting_id=${votingId}`,
      ].join(''),
      result => result.stored_ballots_amount_by_district,
    );
  },
  /**
  ...
  /**
   * Makes ReadRequest returning ballots
   * @param {string} votingId - voting Id
   * @param {string[]} storeTxHashes - store tx hashes
   * @return {ReadPostRequest<Ballot[]>} - ReadPostRequest returning ballots
   */
  getBallotsByStoreTxHashes: (votingId, storeTxHashes) => {
    return new ReadPostRequest(
      ['services/votings_service/v1/ballots-by-store-tx-hashes'],
      {
        voting_id: votingId,
        store_tx_hashes: storeTxHashes,
      },
      response => response.ballots,
    );
  },
  /**
   ...
   * by district map
   */
  getInvalidBallots: (votingId, from, limit) => {
    return new ReadRequest([
      'services/votings_service/v1/invalid-ballots',
      `?voting_id=${votingId}&from=${from}&limit=${limit}`,
    ].join(''));
  },
  /**
   * Makes ReadRequest returning voting results for all districts if voting is finished
   * @param {string} votingId - voting Id
   * @return {ReadRequest<VotingResults>} - ReadRequest returning voting results
   */
  ...
  /**
   * Makes ReadRequest returning voting revote enabled flag
   * @param {string} votingId - voting Id
   * @return {ReadRequest<boolean>} - ReadRequest returning revote enabled flag
   */
  isRevoteEnabled: (votingId) => {
    return new ReadRequest([
      'services/votings_service/v1/revote-enabled',
      `?voting_id=${votingId}`,
    ].join(''));
  },
};
