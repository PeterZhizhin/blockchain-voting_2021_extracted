...
use crate::{
    ...
    types::SealedBoxSecretKeyWrapper,
    util::read_le_u32,
};
...
impl TxPublishDecryptionKey {
    pub fn execute(
        ...
        let decryption_key: box_::curve25519xsalsa20poly1305::SecretKey =
            tx_args.private_key.clone().into();
...
fn test_decryption_key(
    ...
    let decrypted_datums = decrypted_datums.or_else(|_| Err(Error::IncorrectPrivateKey))?;
