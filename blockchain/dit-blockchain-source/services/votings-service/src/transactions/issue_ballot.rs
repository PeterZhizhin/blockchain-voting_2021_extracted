...
use crate::{
    ...
    errors::Error,
    proto,
    schema::{ServiceConfig, VotersRegistry, Voting},
...
impl TxIssueBallot {
    pub fn execute(
        ...
        let mut voters_registry = VotersRegistry::get(context.service_data(), &tx_args.voting_id)
            .ok_or_else(|| Error::VotingDoesNotExist)?;
        voters_registry
            .issue_ballot(tx_args.voter_id.clone(), tx_args.district_id.clone())
            .or_else(|_| Err(Error::BallotCannotBeIssued))?;

        println!(
            "Ballot issued for voter {} in voting {}",
            tx_args.voter_id, tx_args.voting_id
        );
        Ok(())
    }
}
