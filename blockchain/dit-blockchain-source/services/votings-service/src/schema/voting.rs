...
use crate::{
    ...
    errors::Error,
    proto,
    types::{SealedBoxPublicKeyWrapper, SealedBoxSecretKeyWrapper},
};
    ...
    fn into(self) -> CryptoSystemSettingsSchema {
        CryptoSystemSettingsSchema {
            ...
            private_key: match self.private_key {
                ...
                None => box_::curve25519xsalsa20poly1305::SecretKey::from_slice(&[0u8; 32])
                    .unwrap()
                    .into(),
            },
        }
...
impl<T: Access> Voting<T> {
    ...
    fn get_storage(
        access: T,
        voting_id: &String,
    ) -> Result<Entry<T::Base, VotingSchema>, AccessError> {
    ...
    T::Base: RawAccessMut,
{
    ...
    pub fn publish_private_key(
        &mut self,
        private_key: box_::curve25519xsalsa20poly1305::SecretKey,
    ) {
