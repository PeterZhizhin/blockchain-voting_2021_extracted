...
impl<T: Access> VotersRegistry<T> {
    ...
    pub fn get_issued_ballots_amount(&self) -> HashMap<u32, u32> {
        ...
        self.issued_ballots_counter
            .iter()
            .for_each(|(district_id, ballots_amount)| {
    ...
    fn get_voters_list(
        access: T,
        voting_id: &String,
    ) -> Result<MapIndex<T::Base, String, Voter>, AccessError> {
    ...
    fn get_storage(
        access: T,
        voting_id: &String,
    ) -> Result<Entry<T::Base, VotersRegistrySchema>, AccessError> {
        let storages: Group<T, String, Entry<T::Base, VotersRegistrySchema>> = Group::from_access(
            access.clone(),
            IndexAddress::from_root("votings_service").append_name("voters_registry"),
        )?;
    ...
    T::Base: RawAccessMut,
{
    ...
    pub fn register_voters(&mut self, voters_ids: Vec<String>) {
        ...
        voters_ids.iter().for_each(|voter_id| {
            if !self.voters_list.contains(voter_id) {
                self.voters_list
                    .put(&voter_id, Voter::create(voter_id.clone()));
                new_unique_voters += 1;
            }
        });
    ...
    pub fn issue_ballot(&mut self, voter_id: String, district_id: u32) -> Result<(), Error> {
        ...
        let issued_ballots_for_district = self
            .issued_ballots_counter
            .get(&district_id)
            .or(Some(&0))
            .unwrap();
        let issued_ballots_for_district = issued_ballots_for_district + 1;
        self.issued_ballots_counter
            .insert(district_id, issued_ballots_for_district);
