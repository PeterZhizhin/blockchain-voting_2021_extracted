...
pub trait ActualBallotsServiceInterface<Ctx> {
    ...
    fn create_actual_ballots_storage(&self, ctx: Ctx, tx_args: TxCreateActualBallotsStorage) -> Self::Output;
...
impl ActualBallotsServiceInterface<ExecutionContext<'_>>  for ActualBallotsService {
    ...
    fn create_actual_ballots_storage(&self, ctx: ExecutionContext<'_>, tx_args: TxCreateActualBallotsStorage) -> Self::Output {
        TxCreateActualBallotsStorage::execute(&self, ctx, tx_args)
    }

    fn store_grouped_tx_hash(&self, ctx: ExecutionContext<'_>, tx_args: TxStoreGroupedTxHash) -> Self::Output {
        TxStoreGroupedTxHash::execute(&self, ctx, tx_args)
    }

    fn resolve_group_ids(&self, ctx: ExecutionContext<'_>, tx_args: TxResolveGroupIds) -> Self::Output {
        TxResolveGroupIds::execute(&self, ctx, tx_args)
    }

    fn store_actual_ballots(&self, ctx: ExecutionContext<'_>, tx_args: TxStoreActualBallots) -> Self::Output {
